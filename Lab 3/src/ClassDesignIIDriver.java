...
public class ClassDesignIIDriver{
	...
	public static void immutableFractionDriver() {
		...
		//a.setNum(3); //you can't change fractions since their instance
					   //variables are final when created
		//a.setDenom(5);
		...
		a.add(b);
		a = a.add(b); //This one is correct because add() creates a new
					  //fraction instead of changing 'a'
	...
	public static void mutate(int z) {  //does it matter that I called my input data "z" here?
		...
		//Because the line is within a static method. There aren't any object
		// instances in a static method call and 'this' refers to an object
		// instance.
	}
	...
	public static void mutate(Dimension c) {
		...
		//c and b become two names for the same object
		...
		//Yes this does change b of main.
	}
...
When you pass a primitive, you are passing by value which means you are
passing a copy of the data. Changes to the copy aren't reflected in the
original
When you pass an object, you are passing by reference which means you are
passing a reference to the actual parameter. Changes to the copied value will
 be reflected in the original.

When a primitive is passed to a method, does the scope of that primitive
change (i.e., grow to include the called method)?
No the scope does not change.
...
I guess you could say the scope has grown to include the called method. The
parameter of that method becomes another name for that object and the
parameter's scope is within the called method.
 */